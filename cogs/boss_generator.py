...
class BossGenerator(commands.Cog):
    ...
    async def boss_fighter_role(
        ...
        if not role.mentionable and not can_mention_all_roles:
            mention_warning = (
                "\n⚠️ This role is not mentionable. Make it mentionable or grant the bot "
    ...
    async def set_boss_decision(self, guild_id: int, decision: str, user_id: int) -> str:
        ...
        return f"✅ Boss decision set to **{label}**.{sticky_note}"
    ...
    async def boss_report_channel(
        ...
        if channel is None:
            ...
            save_cooldown_config(self.cooldown_config)
            await interaction.response.send_message(
                "✅ Daily guild-boss reports are now disabled. Current counters were preserved.",
                ephemeral=True,
            )
            return
        ...
        if bot_member is None:
            await interaction.response.send_message(
                "❌ I could not check my channel permissions.", ephemeral=True
            )
            return
        permissions = channel.permissions_for(bot_member)
        missing: list[str] = []
        ...
        save_cooldown_config(self.cooldown_config)
        await interaction.response.send_message(
            f"✅ Daily guild-boss reports will be sent in {channel.mention} at the "
